...
### לשונית "ניהול" — ניהול הפרסום הממומן

בתפריט התחתון של ה-CRM יש לשונית **ניהול** (`/crm/manage`) שסורקת את כל מנהל המודעות — קמפיינים ← קבוצות מודעות ← מודעות — ומאפשרת לנהל הכל מהטלפון (דורש טוקן עם `ads_management` בנוסף ל-`ads_read`):

- **מתג הפעלה/השהיה** לכל קמפיין, קבוצת מודעות ומודעה בודדת (השהיה עם שאלת אישור).
- **שינוי תקציב יומי** — ברמת הקמפיין (CBO) או ברמת קבוצת המודעות, לפי איפה שהתקציב מוגדר.
- **מיקוד ערים** — חיפוש ערים בישראל והחלפת המיקוד הגאוגרפי של קבוצת המודעות (כל עיר עם רדיוס ~17 ק״מ, המינימום של פייסבוק; הסרת כל הערים חוזרת לכל הארץ).
- **שכפול קמפיין** כעותק מושהה — הדרך המהירה להוסיף קמפיין חדש על בסיס קמפיין מנצח.
- תצוגת המודעות כוללת תמונות ממוזערות של הקריאייטיב, וכל כרטיס מציג הוצאה/פניות/עלות לפנייה ב-30 הימים האחרונים.
