...
מכל כרטיס קמפיין אפשר גם **לנהל** דרך "ניהול ומיקוד ערים" (דורש טוקן עם `ads_management` בנוסף ל-`ads_read`): השהיה/הפעלה, שינוי תקציב יומי (קמפיין או סט מודעות), **מיקוד ערים** — חיפוש ערים בישראל והחלפת המיקוד הגאוגרפי של סט המודעות (כל עיר עם רדיוס ~17 ק״מ, המינימום של פייסבוק; הסרת כל הערים חוזרת לכל הארץ) — ו**שכפול קמפיין** כעותק מושהה שמשמש כדרך המהירה להוסיף קמפיין חדש על בסיס קמפיין מנצח.
