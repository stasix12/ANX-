...
## 11. גילוי קבוצות (ספטמבר 2026)

דורש `supabase/social-schema-v8.sql` — **חייבים להריץ אותו ב-SQL Editor של
Supabase לפני הכניסה למסך**. בלי זה המסך יציג הודעה בעברית שמסבירה בדיוק את זה,
עם כפתור "נסו שוב"; הוא לא ייתקע בטעינה.

### מה המערכת עושה ומה אתם עושים

לפייסבוק **אין** ממשק שמחפש קבוצות שאתם לא חברים בהן, ואין ממשק ששולח בקשת
הצטרפות. Meta הסירה את Groups API כולו ב-22 באפריל 2024 (הוכרז ב-23 בינואר 2024
עם Graph API v19.0), וביחד איתו את `groups_access_member_info` ואת
`publish_to_groups`. ההרשאות האלה לא קיימות יותר — לא "צריך מנהל שיתקין אפליקציה",
פשוט אין. לכן המסך בנוי כך ש**החיפוש וההצטרפות נעשים אצלכם בדפדפן**, וכל השאר
אוטומטי:

| שלב | מי עושה | איפה |
| --- | --- | --- |
| 1. ביטויי חיפוש מעיר + סוגי קבוצות (כולל Beer Sheva / Be'er Sheva / Arad) | המערכת, לבד | `discovery.ts → buildKeywords()` |
| 2. פתיחת החיפוש של פייסבוק בלחיצה אחת | המערכת מכינה קישור, אתם לוחצים | `discovery.ts → facebookGroupSearchUrl()` |
| 3. **החיפוש עצמו** | **אתם**, בדפדפן | פייסבוק |
| 4. הדבקת מה שמצאתם — קישור אחד, רשימה, או טקסט שלם | אתם מדביקים, המערכת מחלצת ומסננת | `components/social/DiscoveryCapture.tsx` |
| 5. סינון כפילויות מול רשימת הגילוי ומול מאגר הפרסום | המערכת, לבד | `discovery.ts → capturePastedGroups()` |
| 6. קריאת שם / תמונה / מספר חברים / ציבורית-פרטית / האם אתם חברים | המערכת, רק לפי בקשה | `worker/facebook/profile.ts → readGroupDetails()` |
| 7. **ההצטרפות עצמה** | **אתם**, בפייסבוק | פייסבוק |
| 8. סימון "שלחתי בקשה" / "אני חבר" | אתם | המסך |
| 9. העברה למאגר הפרסום | המערכת, לבד | `discovery.ts → promoteToTargets()` → `addGroup()` |

### "בדוק סטטוס" — מה זה באמת

זה **לא** API. זה ה-worker שעל המחשב פותח את דף הקבוצה ב-Chrome שלכם, מחובר
לחשבון שלכם — אותו מנגנון בדיוק שמפרסם עבורכם — וקורא מה שכתוב בדף. קריאה של
קבוצות שאתם לא חברים בהן בולטת יותר מקריאה של הקבוצות שלכם, ולכן:

- זה קורה רק כשאתם מבקשים (opt-in לכל אצווה), אף פעם לא מעצמו.
- שתי קבוצות בכל סבב, עם הפסקה של 6 שניות, ו**רק** כשאין פרסום שממתין — פרסום
  תמיד קודם. בזמן קמפיין פעיל זה לא רץ בכלל.
- שום דבר שם לא לוחץ, לא מקליד, לא מצטרף ולא שולח בקשות.
- פרט שהדף לא אומר במפורש נשאר ריק. מספר חברים מקוצר ("12K", "1.2 אלף") **לא**
  נקלט — לעגל אותו זה להמציא מספר.

**הסימון שלכם תמיד מנצח**: אם סימנתם "אני חבר" והקריאה האוטומטית חושבת אחרת
(קבוצה שרק מנהלים מפרסמים בה נראית בדיוק כמו קבוצה שאתם לא חברים בה), הסימון
שלכם נשמר וההסתירה מוצגת לכם על הכרטיס כהערה.

### כש-Facebook משנה את מסך הקבוצה

הדפוסים של "הצטרפות", "בקשה נשלחה", ציבורית/פרטית ומספר חברים נמצאים
ב-`worker/facebook/selectors.ts` ביחד עם כל השאר. דפוס שלא מזוהה נותן `UNKNOWN`
או ריק — אף פעם לא ניחוש.
