...
קיצור דרך לשולחן העבודה: קליק ימני על `start-worker.cmd` → **שלח אל** → **שולחן העבודה
(צור קיצור דרך)**.
...
Win+R → `shell:startup` → Enter → לגרור לתוך התיקייה קיצור דרך ל-`start-worker.cmd`
(קליק ימני על הקובץ → "צור קיצור דרך"). מעכשיו ה-worker עולה יחד עם המחשב.
