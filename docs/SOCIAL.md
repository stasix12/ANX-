...
## 12. חסמים לפני מכירה ללקוח נוסף (ספטמבר 2026)

הסעיף הזה מתאר **מה שלא תוקן בקוד, ולמה**. כל פריט כאן נבדק בקוד ובסכמה, לא
הוערך. המערכת עובדת היטב לעסק אחד; כל אחד מהסעיפים חוסם מכירה של מושב שני.

### 12.1 חוסם P0 — אין הפרדה בין לקוחות

כל מדיניות RLS על כל טבלת `social_*` היא `to authenticated using (true)`:
`supabase/social-schema.sql:243-259` (לולאה על תשע טבלאות),
`supabase/social-schema-v2.sql:96-111` (`social_workers`, `social_worker_commands`),
`supabase/social-schema-v8.sql:46-54` (`social_content_categories`). אותו דבר
ב-`supabase/crm-schema.sql:62-88` (`leads` — שמות, טלפונים וכתובות של לקוחות).

המשמעות: **כל משתמש מחובר קורא וכותב את כל השורות של כולם** — קבוצות, פוסטים,
תור, היסטוריה, הגדרות. `social_secrets` היא היוצאת מן הכלל והיא בנויה נכון
(RLS פעיל, אפס מדיניות ⇒ רק service-role מגיע אליה, והיא מחזיקה רק ciphertext).

לא נבנתה כאן רב-דיירות, לפי הנחיית הבעלים. מה שנדרש כשיוחלט לבנות:

1. מפתח דייר בכל שורה, וכל מדיניות נגזרת מה-JWT ולא מ-`true` — גם `using` וגם
   `with check`, אחרת אפשר **ליצור** שורה אצל דייר אחר. `using (true)` אחד
   שנשאר על פועל אחד בטבלה אחת פותח את הכל מחדש.
2. האינדקסים הייחודיים צריכים להיות מוגבלי-דייר, אחרת נתונים של לקוחות
   מתנגשים: `social_content_categories` על `lower(name)`,
   `social_queue_slot_idx` על `(schedule_id, target_id, scheduled_at)`, ו-
   `social_settings.key` — שהיום היא **שורה גלובלית אחת לכל מפתח** (`limits`,
   `control`, `business`, `browser`). לקוח שמשנה מכסה יומית או לוחץ "השהה"
   משנה אותה לכולם.
3. שתי ה-buckets צריכות מדיניות לפי נתיב: `social-media` היא `public`
   (Meta מושכת תמונות לפי URL), ו-`social-debug` — צילומי מסך של פייסבוק
   המחובר של הבעלים — קריאה ל-`to authenticated`, כלומר לכל מי שמחזיק את
   התפקיד.
4. `requireAdmin` (`src/lib/social/server/auth.ts:26-33`) מחזיר לכל משתמש תקף:
   אין תפקיד, אין רשימת היתר, אין בדיקת בעלות. ה-routes רצים עם service-role
   שעוקף RLS לגמרי, ולכן הם חייבים לסנן לפי דייר בעצמם.
5. כל מה שה-worker צורך מהטבלאות חייב אימות מחדש אצל ה-worker. אחד כזה כבר
   מתוקן — `worker/adapters/facebookGroupBrowser.ts` מריץ `parseGroupUrl` על
   `social_targets.url` לפני שהוא פותח דפדפן עם ההתחברות החיה לפייסבוק.

**בדיקה שכדאי להריץ עכשיו, גם לפני שמוכרים משהו.** המדיניות מעניקה לפי
**תפקיד**, ולכן כל JWT עם `role: authenticated` מספיק. שתי הגדרות בפרויקט
Supabase מחלקות את התפקיד הזה לציבור — הרשמה פתוחה ב-email, וכניסה אנונימית:

```
curl -s https://<PROJECT>.supabase.co/auth/v1/settings -H "apikey: <ANON_KEY>"
```

`"disable_signup": true` ו-`"external": {"anonymous_users": false}` ⇒ החשיפה
מוגבלת לחשבון המנהל. כל תשובה אחרת ⇒ מפתח ה-anon מפורסם ב-repo
(`.github/workflows/deploy.yml`) והנתונים פתוחים כבר היום. זו הגדרה בלוח
הבקרה של Supabase, לא שינוי קוד.

### 12.2 דרישה לפני מכירה — כל לקוח צריך מחשב משלו

Meta הסירה את Groups API ואת `publish_to_groups` ב-22.4.2024, ולכן אין ולא
יהיה מסלול צד-שרת לקבוצות: `src/lib/social/server/worker.ts` מפרסם רק
`facebook_page` ו-`facebook_group_manual`. קבוצות מפורסמות אך ורק על ידי
`worker/` שמריץ את Chrome של הבעלים עם ההתחברות שלו.

זו עובדה ארכיטקטונית ולא באג, אבל היא חייבת להיאמר **בכתב לפני כל מכירה**:
לכל לקוח נדרש מחשב פרטי שנשאר דלוק ומריץ worker משלו, עם הפרופיל של פייסבוק
שלו. זה גם המצב הנפוץ ביותר בשטח (מחשב שנרדם), ולכן מסך הבית חייב להבדיל בין
"מפרסם עכשיו" ל"אמור היה לפרסם ואיש לא לקח את זה" — ראו `overdue` ב-
`src/lib/social/countdown.ts`.

### 12.3 סיסמת ה-worker היא מפתח לכל המערכת

`worker/env.ts:38-44` + `worker/db.ts:13-19`: ה-worker מתחבר ל-Supabase עם
אימייל וסיסמה של משתמש המנהל מתוך `.env.local`. התכנון עצמו נכון — אין מפתח
service-role על המחשב — אבל בהינתן 12.1, הסיסמה הזו שווה לקריאה וכתיבה של כל
הנתונים, כולל `leads`. במוצר נמכר צריך אישור לכל מכונה, מוגבל-הרשאות ובר-ביטול,
ולא סיסמה אנושית משותפת. **לא ניתן לתקן בלי 12.1.**

### 12.4 טוקן הפרסום של ה-CRM שמור כטקסט גלוי

`src/lib/crm/settings.ts` שומר `accessToken`, `appId` ו-`appSecret` כ-JSON גלוי
ב-`crm_settings`, שעליה אותה מדיניות `using (true)`, ו-
`src/lib/crm/facebookAds.ts` שולח את הטוקן ב-query string. מודול ה-social עושה
את ההפך (AES-256-GCM ב-`social_secrets`, פענוח רק מאחורי service-role).

**לא תוקן כאן בכוונה.** ההעברה ל-`social_secrets` + route בצד שרת היא שינוי
אמיתי בלי מסד נתונים לבדוק מולו, והעברת הטוקן ל-header במקום ל-query string
הופכת את הקריאה ל-preflighted מול Graph ויכולה לשבור את מסך הפרסום — ואי אפשר
לאמת את זה בסביבה הזו. זו משימה לבדה, עם מסד נתונים אמיתי.
