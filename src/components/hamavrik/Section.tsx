...
export function Section({
  ...
  return (
    <section id={id} className={`${bg} py-10 sm:py-14 lg:py-20 ${className}`}>
...
export function SectionHeading({
  ...
  return (
    <Reveal className={`mb-6 flex max-w-2xl flex-col sm:mb-10 ${alignment}`}>
      {eyebrow ? (
      ...
      {lede ? (
        <p className={`mt-2.5 text-[15px] leading-relaxed sm:text-lg ${light ? 'text-white/80' : 'text-mist-300'}`}>{lede}</p>
      ) : null}
    </Reveal>
  );
}
