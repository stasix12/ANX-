...
export function buildRecommendations(
  ...
  for (const c of active) {
    if (c.d30.spend >= MIN_ACTIONABLE_SPEND && c.d30.conversations === 0) {
      items.push({
        ...
        text: `עצור או שפץ את "${c.name}": ${money(c.d30.spend, currency)} ב‑30 יום בלי אף פנייה. עד שיש קריאייטיב או קהל חדשים — התקציב הזה עדיף בסבב אחר.`,
      });
    }
  }
  ...
  if (active.length === 1 && totalSpend >= MIN_ACTIONABLE_SPEND) {
    items.push({
      emoji: '🧺',
      tone: 'watch',
      text: `כל התקציב רץ על סבב פעיל אחד. שווה להריץ במקביל סבב בדיקה קטן (10–15% מהתקציב) עם קהל או קריאייטיב אחרים — כך יש תחליף מוכן כשהסבב הראשי יתעייף.`,
    });
  }
  ...
  if (!items.some((i) => i.tone === 'act')) {
    items.unshift({
      ...
      text:
        accountCpl !== null
          ? `סבבי הפרסום מאוזנים — אין צעד דחוף. עלות פנייה ממוצעת ב‑30 הימים האחרונים: ${money(accountCpl, currency)}. המשך לרענן קריאייטיב כל 3–4 שבועות כדי לשמור עליה.`
          : 'אין עדיין מספיק פניות ב‑30 הימים האחרונים כדי להמליץ על צעד — תן לסבבי פרסום לצבור נתונים.',
    });
  }
